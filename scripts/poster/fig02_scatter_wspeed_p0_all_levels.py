...
measurement height — 50/100/150/200 m match exactly; the model's ~250 m layer
is compared against the mean of the LiDAR's 240 m and 260 m channels (see
``scripts/exploratory/list_model_levels_at_p0.py`` for how these were
identified) — then pools all levels into one density scatter with a 1:1 line,
linear regression, and fit/error stats (N, R, RMSE, bias, and the Takacs 1985
...
def main() -> int:
    ...
    for label_m, model_target_m, obs_h_list in LEVEL_MATCHES:
        ...
        n = len(merged)
        print(f"  level ~{label_m} m (model {model_height:.1f} m): {n} matched records")
        level_counts.append((label_m, n))
        if n == 0:
            continue
        pooled_obs.append(merged["obs"].to_numpy())
        pooled_model.append(merged["model"].to_numpy())
        pooled_level.append(np.full(n, label_m))
    ...
    # Takacs (1985) MSE decomposition (as used in the RBMet reference article):
    # MSE_total = bias^2 + MSE_diss + MSE_disp, exactly (see
    # mse_decomposition() docstring for the identity).
