#!/usr/bin/env python
"""Exploratory map: native-mesh terrain zoomed on an observation site,
highlighting the MPAS cell used for the model-vs-observation comparison.

Same terrain source as the poster figure (``static.nc``: ``ter``/``landmask``)
but zoomed to a small box around the site, with individual Voronoi cells
...
(nearest-neighbour in lat/lon) is visibly justified, not just a reported index.

    python scripts/exploratory/map_p0_nearest_cell.py
    python scripts/exploratory/map_p0_nearest_cell.py --site LPI --margin-km 25
"""
...
def main() -> int:
    ...
    ap.add_argument("--margin-km", type=float, default=15.0,
                     help="half-width of the zoom box around the site, in km (default: 15)")
    ...
    ax.annotate(
        args.site, xy=(site["lon"], site["lat"]), xytext=(8, 8),
    ...
    default_name = f"map_{args.site.lower()}_nearest_cell.png"
