#!/usr/bin/env python
"""Taylor diagram: MPAS vs P0 LiDAR wind speed, per vertical level + pooled.

Same model/obs matching as ``scatter_wspeed_model_vs_p0.py`` (nearest ocean
cell to P0, hourly model paired with the nearest LiDAR record <= 5 min away,
per level: 50/100/150/200 m exact, ~250 m vs mean of LiDAR's 240/260 m), but
summarized on a single normalized Taylor diagram instead of five scatter
plots: each level is one marker at (normalized std dev = std(model)/std(obs),
correlation), plus one marker for all levels pooled together, for reference.
Since each level compares against a slightly different LiDAR subset (masked
bins vary by height), the diagram is normalized per level (reference point
fixed at (1, 1)) rather than using one shared absolute reference std.

    python scripts/exploratory/taylor_wspeed_p0.py
"""
...
def main() -> int:
    ...
    for label_m, model_target_m, obs_h_list in LEVEL_MATCHES:
        ...
        n = len(merged)
        print(f"  level ~{label_m} m: {n} matched records")
        if n == 0:
            continue
        x = merged["obs"].to_numpy()
        y = merged["model"].to_numpy()
        stdratio, corr = taylor_stats(x, y)
        points.append((f"{label_m} m", stdratio, corr, n))
        pooled_obs.append(x)
    ...
    # Reference point (obs vs itself: std ratio 1, correlation 1).
    ...
    # Dashed reference arc at std ratio = 1.
    theta_arc = np.linspace(0, np.pi / 2, 100)
    ax_polar.plot(theta_arc, np.ones_like(theta_arc), "k--", lw=0.8, alpha=0.6)

    # Centered-RMS contours (dotted), matching the RMSE' = MSE_diss + MSE_disp
    # decomposition used in the scatter figures (Takacs 1985 / Taylor 2001).
    ...
    ax_polar.legend(loc="upper left", bbox_to_anchor=(1.05, 1.0), fontsize=9,
                    frameon=True)
    fig.suptitle(f"Taylor diagram — MPAS {sim.label} vs P0 wind speed", fontsize=13)
    fig.tight_layout()

    FIG_DIR.mkdir(parents=True, exist_ok=True)
    out = args.output or FIG_DIR / "taylor_wspeed_p0.png"
