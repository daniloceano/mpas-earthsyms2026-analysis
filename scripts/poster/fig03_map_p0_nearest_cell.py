#!/usr/bin/env python
"""Poster Figure 3 / banner: native-mesh terrain zoomed on P0, highlighting the
MPAS cell used for the model-vs-observation comparison.

Same terrain source as fig01 (``static.nc``: ``ter``/``landmask``) but zoomed
to a small box around the P0 LiDAR site, with individual Voronoi cells
outlined (few enough at this zoom to be legible) so the specific cell picked
by ``scatter_wspeed_model_vs_p0.py``/``list_model_levels_at_p0.py``
(nearest-neighbour in lat/lon) is visibly justified, not just a reported
index. The highlighted cell is outlined only (no fill), so its land/ocean
color stays visible underneath — confirming at a glance that the comparison
cell is ocean, not land.

    python scripts/poster/fig03_map_p0_nearest_cell.py
    python scripts/poster/fig03_map_p0_nearest_cell.py --margin-km 25
"""
...
def main() -> int:
    ...
    ap.add_argument("--margin-km", type=float, default=15.0,
                     help="half-width of the zoom box around P0, in km (default: 15)")
    ...
    print(f"{len(box_idx)} cells within {args.margin_km:.0f} km of P0")
    ...
    ax.annotate(
        "P0", xy=(site["lon"], site["lat"]), xytext=(8, 8),
    ...
    out = args.output or FIG_DIR / "fig03_map_p0_nearest_cell.png"
